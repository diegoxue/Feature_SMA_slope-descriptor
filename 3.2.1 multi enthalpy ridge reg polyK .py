    ...
    for i in range(100):
        ...
        for m in range(10):
            krr_poly = RandomizedSearchCV(KernelRidge(), params_poly, cv=10, verbose=0,
                                          ...
                                          random_state=m, n_jobs=-1)  # cv=10即10折交叉验证
...
if __name__ == '__main__':
    ...
    data_enthalpy =pd.read_pickle('enthalpy_tini')
    enthalpy_slope = pd.read_pickle('enthalpy_slope_tini')
    slope_imp = enthalpy_slope.loc[:,slope_str]
    Slope = pd.read_pickle('Slope')
    Feature = ['slope_imp','mr','ar_c','en','anum','ven','ea','dor','mass','volume','energy1','CE','EBE','YM','cs','Tm','cv']
    data_enthalpy.iloc[:,1] = slope_imp
    data_enthalpy.rename(columns = {'slope1':'slope_imp'}, inplace=True)
    x_slope = data_enthalpy['slope_imp'].values.reshape(-1,1)
    y = data_enthalpy['enthalpy'].values.reshape(-1,1)
    ...
    for m in tqdm(range(4,7)):
        ...
        for i in tqdm(Feature_inter):
            if 'slope_imp' not in i and m > 1:
                continue
            else:
                # 分析hp的情况
                X = data_enthalpy.loc[:,i]
                X_scaler = StandardScaler().fit_transform(X)
                y = data_enthalpy.loc[:,'enthalpy']
                ...
                #         bestscore = krr_poly.best_score_
                #         model = krr_poly.best_estimator_
                results = RidgeResample(X_scaler,y)
