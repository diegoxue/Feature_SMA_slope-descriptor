...
if __name__ == '__main__':
    ...
    data_hysteresis =pd.read_pickle('hysteresis_tini2')
    hysteresis_slope = pd.read_pickle('hysteresis_slope_tini2')
    slope_imp = hysteresis_slope.loc[:,slope_str]
    Slope = pd.read_pickle('Slope')
    Feature = ['slope_imp','mr','ar_c','en','anum','ven','ea','dor','mass','volume','energy1','CE','EBE','YM','cs','Tm','cv']
    data_hysteresis.iloc[:,1] = slope_imp
    data_hysteresis.rename(columns = {'slope1':'slope_imp'}, inplace=True)


    # 比较第15代斜率特征和其他特征的多元RR和LR拟合对比
    # 组合1-6个特征进行岭回归，并提取前15特征组合
    CompTrainMean,CompTestMean,CompTrainStd,CompTestStd,Compk = [],[],[],[],[]
    for m in tqdm(range(4,5)):
        ...
        for i in tqdm(Feature_inter):
            if 'slope_imp' in i and m > 1:
                continue
            else:
                X = data_hysteresis.loc[:,i]
                X_scaler = StandardScaler().fit_transform(X)
                y = data_hysteresis.loc[:,'hysteresis']
